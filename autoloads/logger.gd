...
class ExternalSink:
	...
	func _init(_name: String, _queue_mode := QUEUE_MODES.NONE) -> void:
		name = _name
...
class Logfile:
	...
	func _init(_path: String, _queue_mode := QUEUE_MODES.NONE) -> void:
	...
	func validate_path(p_path: String) -> bool:
		...
		if not (p_path.is_absolute_path() or p_path.is_relative_path()):
			print("[ERROR] [logger] The given path '%s' is not valid." % p_path)
			return false
		var base_dir := p_path.get_base_dir()
	...
	func _open_file(p_path: String) -> FileAccess:
		var result := FileAccess.open(path, get_write_mode())

		if result == null:
			var err := FileAccess.get_open_error()
			print("[ERROR] [logger] Could not open the '%s' log file; exited with error %d." % [p_path, err])
			return null
		else:
			return result
...
class Module:
	...
	func _init(_name: String, _output_level: int, _output_strategies: Array, _external_sink: ExternalSink) -> void:
		name = _name
		...
		if typeof(_output_strategies) == TYPE_INT: # Only one strategy, use it for all
			...
			for strategy: int in _output_strategies: # Need to force deep copy
	...
	func set_output_strategy(output_strategy_mask: int, level := -1) -> void:
	...
	func get_output_strategy(level := -1) -> Variant:
		if level == -1:
...
func put(level: int, message: String, module := default_module_name, error_code := -1) -> void:
...
func verbose(message: String, module := default_module_name, error_code := -1) -> void:
	"""Log a message in the given module with level VERBOSE."""
	put(VERBOSE, message, module, error_code)


func debug(message: String, module := default_module_name, error_code := -1) -> void:
	"""Log a message in the given module with level DEBUG."""
	put(DEBUG, message, module, error_code)


func info(message: String, module := default_module_name, error_code := -1) -> void:
	"""Log a message in the given module with level INFO."""
	put(INFO, message, module, error_code)


func warn(message: String, module := default_module_name, error_code := -1) -> void:
	"""Log a message in the given module with level WARN."""
	put(WARN, message, module, error_code)


func error(message: String, module := default_module_name, error_code := -1) -> void:
...
func add_module(p_name: String, output_level := default_output_level, output_strategies := default_output_strategies, logfile: Logfile = null) -> Module:
	...
	if modules.has(p_name):
		info("The module '%s' already exists; discarding the call to add it anew." % p_name, PLUGIN_NAME)
	else:
		if logfile == null:
			logfile = get_external_sink(default_logfile_path)
		modules[p_name] = Module.new(p_name, output_level, output_strategies, logfile)
	return modules[p_name]


func get_module(module := default_module_name) -> Module:
...
func set_default_logfile_path(new_logfile_path: String, keep_old := false) -> void:
...
func add_logfile(logfile_path := default_logfile_path) -> Logfile:
...
func get_external_sinks() -> Dictionary:
...
func set_default_output_strategy(output_strategy_mask: int, level := -1) -> void:
...
func format(template: String, level: int, module: String, message: Variant, error_code := -1) -> String:
	var output := template
	...
	var error_message: Variant = ERROR_MESSAGES.get(error_code)
...
func save_config(configfile: String = default_configfile_path) -> Error:
	...
	for external_sink: String in sorted_keys:
	...
	sorted_keys.sort()
	for module: String in sorted_keys:
...
func load_config(configfile: String = default_configfile_path) -> Error:
	...
	for logfile_cfg: Variant in config.get_value(PLUGIN_NAME, config_fields.external_sinks, []):
	...
	for module_cfg: Variant in config.get_value(PLUGIN_NAME, config_fields.modules, []):
