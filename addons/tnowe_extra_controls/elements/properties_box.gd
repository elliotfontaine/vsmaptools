...
func add_group(label : String, is_folded: bool = false):
	...
	title.toggled.connect(func(toggled_on):
		if toggled_on: title.icon = get_theme_icon(&"arrow", &"Tree")
		else: title.icon = get_theme_icon(&"arrow_collapsed", &"Tree")
	)
	...
	title.button_pressed = !is_folded
...
func set_value_at(index : int, value: Variant) -> void:
	...
	elif editor is TextEdit and value is String: editor.text = value
